...
采用 Leader-Follower 模式避免线程竞争。当从 DelayQueue 获取元素时，如果第一个元素已经过期，就直接将其返回；若还未过期，就通过 await 进行等待。其中 Leader 角色的线程等待时间为过期时间，其他线程的等待时间为无限期，当 Leader 成功获取到头元素后，会唤醒其他的等待线程，继续进行下一次元素获取流程。通过这种方式来避免多个线程之间不必要的竞争。
